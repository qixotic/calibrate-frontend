...
- `src/components/human-labelling/` — labelling-task dialogs. `AddRunToLabellingTaskDialog.tsx` powers the "Submit for labelling" action on three result surfaces, each mapping to one task type via `buildItemsFromSource(source)` (the source discriminated union drives the target type via `targetTaskTypeForSource`; you never pick it):
