...
- `src/components/agent-tabs/` — the tabbed UI on `/agents/[uuid]` (Agent / Tools / Data Extraction / Evaluators / Tests / Traces / Settings for build agents; Connection / Evaluators / Tests / Traces / Settings for connection agents). Which tabs appear is data-driven via `calibrateTabs` / `connectionTabs` arrays and a `tabLabels` map.
  - The **Traces tab** (`TracesTabContent`) shows what this agent actually did in production — one row per turn, ingested by the customer's own backend via `POST /traces`. **Traces belong to an agent, not to the workspace**: every read, count, and delete goes out with `agent_id`, there is no workspace-level `/traces` page or sidebar item, and bulk-delete passes `agent_id` so "delete all matching" cannot reach another agent's traces. The empty state renders a copy-paste `curl` carrying the agent's real UUID, which is the one value a customer has to hardcode and the only place in the app to read it. Server-paginated via `useTraces` (search and filters round-trip to the backend rather than filtering a fully-fetched list, since the trace store can far outgrow what the client should download). A banner warns when the workspace has filled its trace limit — it keys off a **workspace-wide** count (`useWorkspaceTraceCount`), never the per-agent count already on screen, because the limit is workspace-scoped and an agent holding a handful of traces can still be refused. Each row also carries an **evaluations badge** (`eval_summary` on the list row) opening `TraceEvaluationsDialog`; `eval_summary: null` means nothing has judged the trace yet and is deliberately NOT the same as `{passed: 0, total: n}`, so the two are told apart by wording, border, and clickability rather than colour alone.
