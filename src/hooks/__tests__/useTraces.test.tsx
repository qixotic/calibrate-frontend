...
describe("useTraces", () => {
  it("loads the first page on mount and exposes items + total", async () => {
    ...
    const { result } = renderHook(() =>
      useTraces({ accessToken: "tok", agentUuid: AGENT, q: "", conversationId: null }),
    );
  ...
  it("stays idle without an access token", async () => {
    const { result } = renderHook(() =>
      useTraces({ accessToken: null, agentUuid: AGENT, q: "", conversationId: null }),
    );
...
describe("useTraceCount", () => {
  it("reads the envelope total with a limit=1 probe", async () => {
    ...
    const { result } = renderHook(() => useTraceCount("tok", AGENT));
...
describe("agent scoping", () => {
  it("sends the agent on every page fetch", async () => {
    mockFetchTraces.mockResolvedValue(page([], 0));
    ...
    expect(mockFetchTraces.mock.calls[0][1].agentUuid).toBe("agent-9");
  });

  it("refetches from page one when the agent changes", async () => {
    mockFetchTraces.mockResolvedValue(page([], 0));

    const { rerender } = renderHook(
      ({ agentUuid }) =>
        useTraces({ accessToken: "tok", agentUuid, q: "", conversationId: null }),
      { initialProps: { agentUuid: "agent-a" } },
    );
    await waitFor(() => expect(mockFetchTraces).toHaveBeenCalledTimes(1));

    rerender({ agentUuid: "agent-b" });

    await waitFor(() =>
      expect(
        mockFetchTraces.mock.calls.some((c) => c[1].agentUuid === "agent-b"),
      ).toBe(true),
    );
    const last = mockFetchTraces.mock.calls.at(-1)[1];
