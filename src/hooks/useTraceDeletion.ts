...
type UseTraceDeletionArgs = {
  ...
  /** The agent these traces belong to. Sent so the backend bounds the delete
   *  to that agent even if a stale id from another agent reaches the list. */
  agentUuid: string;
};
...
// batched by design).
function buildRequest(backendUrl: string, uuids: string[], agentUuid: string) {
  return {
    url: `${backendUrl}/traces/bulk-delete`,
    method: "POST",
    body: JSON.stringify({ trace_ids: uuids, agent_id: agentUuid }),
  };
}
...
export function useTraceDeletion({
  ...
  return useBulkDeletion<TraceSummary>({
    ...
    buildBulkRequest: (backendUrl, uuids) =>
      buildRequest(backendUrl, uuids, agentUuid),
    buildSingleRequest: (backendUrl, uuid) =>
      buildRequest(backendUrl, [uuid], agentUuid),
  });
}
