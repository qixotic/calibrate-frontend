...
export type TraceSummary = {
  ...
  /** Distinct tool names, first-seen order, truncated by the backend — so this
...
export type TraceListParams = {
  ...
  /** Restrict to one agent. Traces are always read from an agent's tab, so in
...
export async function fetchTraces(
  accessToken: string,
  { limit, offset, q, conversationId, agentUuid }: TraceListParams,
  ...
  if (agentUuid) params.set("agent_id", agentUuid);
...
export async function bulkDeleteMatchingTraces(
  accessToken: string,
  {
    ...
    conversationId,
    agentUuid,
  }: { q?: string; conversationId?: string; agentUuid?: string },
): Promise<BulkDeleteTracesResult> {
  const body: Record<string, unknown> = { select_all: true };
  if (q && q.trim()) body.q = q.trim();
  if (conversationId) body.conversation_id = conversationId;
  // Without this the backend would match every agent's traces, not just the
  // one whose tab the button was pressed on.
  if (agentUuid) body.agent_id = agentUuid;
