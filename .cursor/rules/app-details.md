...
- **Automatic scoring (opt-in).** The Traces tab has a **Score new traces automatically** switch driven by `agents.auto_score_traces` (`PUT /agents/{uuid}` with only that field). It scores traces ingested **after** it is turned on, not ones already stored. Before enabling, the tab fetches JWT-only `GET /agents/{uuid}/trace-scoring-eligibility` and shows which linked evaluators can score and why the rest cannot (`wrong_type_for_agent`, `no_live_version`, `declares_variables`, in ordinary words). Enabling is hard-disabled when `eligible` is empty. Turning scoring **off** stays available even if eligibility later drifts. There is no rescore or backfill control on this surface.
